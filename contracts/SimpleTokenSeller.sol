...
contract SimpleTokenSeller {
  ...
  constructor (ERC20Mintable _token, address _trustedClaimHolder) public
  {
    token = _token;
  ...
  function () external payable {
    getTokens(msg.sender, msg.value);
  }

  function getTokens(address _who, uint256 _weiAmount) public
  {
    require(_who != address(0));
    require(_weiAmount != 0);

    ClaimHolder beneficiaryIdentity = ClaimHolder(_who);
